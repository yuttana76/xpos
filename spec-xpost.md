...
- `GET reports/daily-sales/?from&to` — **new 2026-08-05** — revenue/order-count bucketed **per day** (zero-filled for every day in range, same zero-fill pattern as `sales-by-hour`'s 24-hour fill), merged across accessible stores (no `by_store` split, matching the 3 non-`sales/` report endpoints). Capped at 366 days (`DailySalesReportView.MAX_DAYS`) to bound the zero-fill loop. Powers the shared `DailyRevenueChart` component embedded in `/reports/sales`, `/reports/menu-performance`, `/reports/sales-by-hour` (§9, §11.1) — **not** `/reports/sales-by-staff`, which uses the per-staff variant below instead. `IsOwnerOrManager`.
- `GET reports/daily-sales-by-staff/?from&to` — **new 2026-08-05** — same per-day zero-fill as above, but **one series per staff member who received ≥1 payment in range** (staff with zero sales in the whole range are omitted from the chart entirely — they'd just be a flat line at 0 cluttering the legend; they're still visible in `sales-by-staff/`'s table). Capped at 8 series total (`DailySalesByStaffReportView.MAX_SERIES`, matching the dataviz palette's categorical cap): top 7 staff by total revenue get their own line, everyone else folds into a combined "อื่นๆ" line — never a synthesized 9th hue. Powers `DailyRevenueByStaffChart` on `/reports/sales-by-staff` only. `IsOwnerOrManager`.
...
/floor                     → table grid + kitchen-status panel (split into "โต๊ะ"/"Takeaway"
                             ...
                             present — added 2026-07-31; PENDING/SENT/SERVED filter chips —
                             added 2026-08-05, see §19) + revenue summary tiles — as of
                             2026-08-05, **all `฿` amounts on this page (รายรับวันนี้, รายรับ
                             เดือนนี้, the by-store table) are OWNER-only**, tightened from
                             OWNER/MANAGER (monthly tile) and all-roles (today tile). Scoped to
                             this page specifically — the Sidebar's "รายรับวันนี้" nav link and
                             `/reports/today` itself are unchanged and still open to every role
                             (§7, §14 — a separate, deliberate product decision not to be
...
Persistent left nav (desktop: static column; mobile: hamburger + slide-over drawer with backdrop). Role-filtered nav items. Contains an **expandable "รายงาน" sub-menu** (auto-expands if the current route is a report sub-page), grouped by category (ยอดขาย / ตรวจสอบ / ภาษี) matching the `/reports` hub categorization. Hidden entirely on `/`, `/login`, `/order-session/*`. **Reads `getStaffSession()` inside a `useEffect`, not synchronously during render** (fixed 2026-08-05 — see §18) — server has no `localStorage`, so a synchronous read made the client's first render diverge from the server-rendered HTML on every fresh full page load while logged in, a React hydration error. `StatusBar.tsx` had the identical bug and got the identical fix; both now start `session` as `null` and populate it post-mount, matching the pre-existing pattern this file already used for `collapsed`.
...
### `frontend/src/components/DailyRevenueChart.tsx` — **new 2026-08-05**
Self-contained (owns its own fetch/loading state, only needs `{from, to}` props) daily-revenue **line chart**, embedded in `/reports/sales`, `/reports/menu-performance`, `/reports/sales-by-hour` (not `/reports/sales-by-staff` — see the per-staff variant below), fetching `GET /api/orders/reports/daily-sales/`. Hand-rolled SVG (no charting library added — none was installed; kept in the same zero-dependency spirit as `/reports/sales-by-hour`'s existing hand-rolled CSS bar chart). 2px line, round joins, ~10%-opacity area wash, hairline gridlines with clean rounded/comma'd y-axis ticks, sparse x-axis date labels (caps at ~7 to avoid overlap on wide ranges), a value-at-the-end direct label, and a pointer-following crosshair + tooltip (nearest-point snap). Single series → no legend (card title names it). A "แสดงตาราง" toggle reveals the same data as a plain table (date/revenue/order-count), satisfying the accessibility requirement that every charted value stay reachable without hovering. Built per the project's `dataviz` skill procedure; color validated via `scripts/validate_palette.js "#38bdf8" --mode dark --surface "#0f172a"` (exit 0).

### `frontend/src/components/DailyRevenueByStaffChart.tsx` — **new 2026-08-05**
Multi-series sibling of `DailyRevenueChart`, used **only** on `/reports/sales-by-staff`, fetching `GET /api/orders/reports/daily-sales-by-staff/` — one line per staff member (per backend's 8-series cap, §7). Same mark specs as the single-series chart, plus what multi-series requires per the `dataviz` skill: a legend row (fixed fresh-per-render categorical color order — blue/orange/aqua/yellow/magenta/green/violet/red, dark-mode steps, validated together via `validate_palette.js` for all 8 at once), a shared y-scale across all series, and **one tooltip listing every series' value at the hovered date** (not just whichever line the pointer happens to be over) with a short line-key stroke per row instead of a filled swatch. No per-point direct labels (would collide with up to 8 converging lines) — legend + tooltip + table carry identity/values instead, per the skill's documented fallback for that case. "แสดงตาราง" here renders a date×staff matrix (sticky first column) rather than a flat list, since there's one value per staff per day.
...
## 19. Changelog — 2026-08-05 batch

1. **Daily-revenue line chart added to 4 report pages** (`/reports/sales`, `/reports/menu-performance`, `/reports/sales-by-hour`, `/reports/sales-by-staff`) — new shared `DailyRevenueChart` component (§9) backed by a new `GET /api/orders/reports/daily-sales/` endpoint (§7). Built following the project's `dataviz` skill (mark specs, hover/crosshair, table-view accessibility fallback, palette-validated color) — no charting library added, hand-rolled SVG matching the zero-dependency precedent already set by `/reports/sales-by-hour`'s bar chart. Verified visually with Playwright screenshots (populated chart, hover tooltip, table toggle) across all 4 pages, not just type-checked.
2. **Fixed a real, previously-undiagnosed hydration bug affecting every report page** (and every page rendering `Sidebar`/`StatusBar` in general) — found incidentally while visually verifying the chart above. `Sidebar.tsx` and `StatusBar.tsx` both read `getStaffSession()` **synchronously during render** to decide what to show; since the server has no `localStorage`, every *fresh full page load* (not client-side navigation — that path never re-hydrates) while logged in produced a React hydration mismatch (console error, discarded SSR output, silent extra re-render). Reproduced independently of the chart work (confirmed on `/reports/today`, untouched by this batch) — not something introduced by the chart, but affecting the very pages this batch shipped, so fixed here. Both components now start `session` as `null` and populate it in a `useEffect`, the exact pattern `Sidebar.tsx` already used for its `collapsed` state — see the inline comment there for why. Confirmed fixed via Playwright: zero console errors on a fresh full-page load of all 4 report pages post-fix (vs. the error reproducing pre-fix).
3. **`/reports/sales-by-staff` chart replaced with a per-staff breakdown** (§7, §9) — the aggregate single-line chart from item 1 above made little sense on the one report page whose entire purpose is comparing staff, so it was swapped for `DailyRevenueByStaffChart` (one line per staff member, backed by the new `reports/daily-sales-by-staff/` endpoint) on that page only; the other 3 pages from item 1 keep the aggregate chart unchanged. Zero-sales staff are omitted from the chart/legend (still visible in the existing staff table below it) and a >8-staff store folds the smallest earners into an "อื่นๆ" line, keeping the series count within the categorical palette's validated cap. Verified visually via Playwright: legend, multi-line rendering, the "every series in one tooltip" hover behavior, and the date×staff matrix table view all confirmed working against real seeded data (two staff with actual paid orders on different days).
4. **`/floor` kitchen-status panel gained PENDING/SENT/SERVED filter chips** (§9) — the existing static color-dot legend (§18 item 8) became clickable toggles; deselecting a status hides matching items from every order card and hides a card entirely once none of its items match, across both the "โต๊ะ" and "Takeaway" sections. Default (all three on) reproduces the prior unfiltered behavior exactly. Two distinct empty-state messages depending on *why* nothing is showing ("ยังไม่ได้เลือกสถานะที่จะแสดง" when the filter itself is empty vs. "ไม่มีรายการที่ตรงกับตัวกรองที่เลือกอยู่" when items just don't match) so staff aren't left wondering whether it's a bug or their own filter choice.
5. **Fixed two more hydration-adjacent bugs found while building/verifying the filter above** (`frontend/src/app/floor/page.tsx`) — same root cause class as item 2, but two distinct new instances:
   - `useState(getStaffSession())` (a **non-lazy** initializer, evaluated fresh on the very first render) had the identical server/client mismatch as `Sidebar`/`StatusBar`'s bug — except this was the *only* remaining occurrence of that exact pattern anywhere in the frontend (confirmed via `grep -rln "useState(getStaffSession())"`). Fixed the same way: start `session` at `null`, populate in a `useEffect`.
   - That fix alone introduced a **new** regression: splitting "populate session" and "redirect to `/login` if no session" into two separate `useEffect`s made both fire in the same mount pass seeing the pre-update `session === null`, so a *logged-in* user hitting `/floor` via a fresh full page load got bounced straight back to `/login` (caught by an automated Playwright pass — a full-page-load smoke test, not just `tsc`, would have caught this immediately; a unit test would not have, since the bug only exists across a fresh mount + effect-ordering interaction). Fixed by merging both back into **one** effect that reads the session once and either redirects or sets state — never both effects observing the same stale value.
6. **`/floor`'s revenue tiles restricted to OWNER only** — previously รายรับวันนี้ was shown to every role and รายรับเดือนนี้/the by-store table to OWNER **or** MANAGER; all three now gate on `session?.staff.role === "OWNER"` exclusively (§9). Deliberately scoped to this one page — `/reports/today` and the Sidebar's "รายรับวันนี้" link are untouched and remain open to every role, an existing and separate product decision (§7, §14). Verified visually: PIN `1111` (OWNER) still sees all three; PIN `2222` (confirmed via the API's own `staff.role` field to be `SERVER`) sees none of them, with the operational tiles (บิลที่ชำระแล้ว/สถานะโต๊ะ/รายการรอส่งครัว) still present.
7. **Fixed `Sidebar`/`StatusBar` showing stale role-gated content after logout → re-login as a different role, with no page reload in between** — a direct consequence of item 2's own fix: reading `session` in a `useEffect` with `[]` deps means it only ever runs once per mount, but `Sidebar`/`StatusBar` live in the root layout and **never unmount** across client-side navigation (unlike page components, which do — this is why `/floor`'s own revenue-tile role-gating from item 6 was never affected, only the persistent chrome around it). Reported directly by the user testing exactly this sequence. Fixed by keying the session-reading effect on `[pathname]` instead of `[]` — login/logout always navigates to a different route (`/login` ↔ `/floor`), so a pathname change is a reliable, already-available signal to re-check, without introducing a separate session-change event/context. Verified via Playwright reproducing the literal reported scenario end-to-end (log in as OWNER via a fresh page load, confirm OWNER-only nav item present → click the app's own logout button → log back in as `SERVER` with **no page reload** → confirm the OWNER-only nav item is now gone and the role label reads `SERVER`).
